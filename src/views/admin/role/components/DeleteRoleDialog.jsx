...
import { deleteRole } from '@/stores/RoleSlice'
import { TrashIcon } from '@radix-ui/react-icons'
import { useDispatch, useSelector } from 'react-redux'

const DeleteRoleDialog = ({ role, showTrigger = true, ...props }) => {
  const dispatch = useDispatch()
  const loading = useSelector((state) => state.role.loading)

  const destroy = async (data) => {
    try {
      await dispatch(deleteRole(data)).unwrap()
    } catch (error) {
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            <strong>{role.name}</strong> sẽ bị xóa
          </DialogDescription>
        </DialogHeader>
