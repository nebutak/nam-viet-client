...
import { createTax } from '@/stores/TaxSlice'
import { createTaxSchema } from '../schema'

const CreateTaxDialog = ({
  open,
  onOpenChange,
  showTrigger = true,
  ...props
  ...
  const onSubmit = async (data) => {
    try {
      await dispatch(createTax(data)).unwrap()
      form.reset()
      onOpenChange?.(false)
    } catch (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange} {...props}>
      ...
      <DialogContent className="md:h-auto md:max-w-2xl">
        <DialogHeader>
          <DialogTitle>Thêm loại thuế mới</DialogTitle>
          <DialogDescription>
            Điền vào chi tiết phía dưới để thêm loại thuế mới
          </DialogDescription>
        </DialogHeader>

        <div className="max-h-[65vh] overflow-auto md:max-h-[75vh]">
          <Form {...form}>
            <form id="create-tax" onSubmit={form.handleSubmit(onSubmit)}>
        ...
        <DialogFooter className="flex gap-2 sm:space-x-0">
          ...
          <Button form="create-tax" loading={loading}>
            Thêm mới
          </Button>
        </DialogFooter>
      </DialogContent>
