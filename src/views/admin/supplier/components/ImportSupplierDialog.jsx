    ...
    const handleImport = async () => {
        ...
        try {
            ...
            worksheet.eachRow((row, rowNumber) => {
                ...
                const item = {
                    supplierType: String(getVal(1) || 'local').trim() === 'Trong nước' ? 'local' : String(getVal(1)).trim(),
                    ...
                    paymentTerms: String(getVal(8)).trim(),
                    note: String(getVal(9)).trim(),
                    status: String(getVal(10)).trim() === 'Ngừng giao dịch' || String(getVal(10)).trim() === 'Khóa' ? 'inactive' : 'active',
                }
            ...
            } else if (error?.response?.data?.error?.importErrors && Array.isArray(error.response.data.error.importErrors)) {
                importErrors = error.response.data.error.importErrors
            }
