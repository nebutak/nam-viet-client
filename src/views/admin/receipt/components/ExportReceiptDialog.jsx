...
import ExportReceiptView from './ExportReceiptView'
import { IconPresentationAnalytics } from '@tabler/icons-react'

const ExportReceiptDialog = ({
  open,
  onOpenChange,
  showTrigger = true,
  isMyReceipt = false,
  ...
  const handleReviewExport = async () => {
    setLoading(true)
    try {
      const url = isMyReceipt ? '/payment-vouchers/my-payment-vouchers' : '/payment-vouchers'
      ...
      if (!list.length) {
        toast.warning('Danh sách phiếu thu trống')
        return
      }
      setExportData(list)
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange} {...props}>
      ...
      <DialogContent className="md:h-auto md:w-[320px]">
        <DialogHeader>
          <DialogTitle>Xuất báo cáo phiếu thu</DialogTitle>
          <DialogDescription>
...
export default ExportReceiptDialog
