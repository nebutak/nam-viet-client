...
import { deleteCustomer } from '@/stores/CustomerSlice'
import { TrashIcon } from '@radix-ui/react-icons'
import { useDispatch, useSelector } from 'react-redux'

const DeleteCustomerDialog = ({ customer, showTrigger = true, contentClassName, overlayClassName, ...props }) => {
  const dispatch = useDispatch()
  const loading = useSelector((state) => state.customer.loading)

  const destroy = async (data) => {
    try {
      await dispatch(deleteCustomer(data)).unwrap()
    } catch (error) {
      ...
      <DialogContent className={contentClassName} overlayClassName={overlayClassName}>
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            <strong>{customer.name}</strong> sẽ bị xóa
          </DialogDescription>
        </DialogHeader>
